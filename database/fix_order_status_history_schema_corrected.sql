-- Fix order_status_history table schema (Corrected Version)
...
-- Step 2: Add missing status_type column if it doesn't exist
DO $$
BEGIN
...
-- Step 3: Update existing records to have a default status_type
UPDATE public.order_status_history 
SET status_type = 'manual_update'
WHERE status_type IS NULL;

-- Step 4: Make status_type NOT NULL with a default value
ALTER TABLE public.order_status_history 
ALTER COLUMN status_type SET DEFAULT 'manual_update',
ALTER COLUMN status_type SET NOT NULL;

-- Step 5: Add any other missing columns that might be needed
DO $$
BEGIN
...
-- Step 6: Create or update the updated_at trigger
...
-- Step 9: Test inserting a record (CORRECTED - includes changed_by field)
DO $$
DECLARE
    test_order_id UUID;
    test_seller_id UUID;
...
BEGIN
    ...
    IF test_order_id IS NOT NULL AND test_seller_id IS NOT NULL THEN
        ...
        RAISE NOTICE 'SUCCESS: Test record inserted with ID: %', test_record_id;
