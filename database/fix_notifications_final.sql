...
-- The issue is that RLS is still blocking notification creation even with SECURITY DEFINER

-- Step 1: Completely disable RLS temporarily to test
ALTER TABLE public.notifications DISABLE ROW LEVEL SECURITY;

-- Step 2: Drop all existing policies
DROP POLICY IF EXISTS "Allow system notifications" ON public.notifications;
...
-- Step 4: Ensure the create_notification function exists and works
...
-- Step 5: Create the notify_new_order function
...
-- Step 6: Create the notify_order_status_update function
...
-- Step 7: Drop existing triggers and recreate them
...
-- Step 9: Test the fix by creating a test notification
DO $$
DECLARE
    test_user_id UUID;
    notification_id UUID;
BEGIN
    ...
    IF test_user_id IS NOT NULL THEN
        ...
        SELECT create_notification(
            test_user_id,
            'new_order_received',
            'Test Notification - Final Fix',
            'This is a test to verify the final fix works',
...
SELECT 'Final Notification Fix Applied Successfully' as status;
